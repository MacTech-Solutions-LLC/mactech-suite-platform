...
import {
  ...
  verifySuiteObjectReference,
} from "@/lib/suite-object-reference";
import {
  validationIssues,
  verifySuiteObjectReferenceSchema,
...
export async function POST(request: NextRequest) {
  ...
  const parsed = verifySuiteObjectReferenceSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const reference = await verifySuiteObjectReference(
      {
        ...input,
