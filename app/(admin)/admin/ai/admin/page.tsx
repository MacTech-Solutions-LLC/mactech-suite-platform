...
export default async function AiAdminPage() {
  ...
  return <div className="space-y-6"><PageHeader title="MacTech AI administration" description="Sanitized provider, policy, retrieval, and tool status. Provider secrets are never displayed." /><div className="grid gap-4 md:grid-cols-2">{rows.map(([label, value]) => <div key={label} className="rounded-lg border border-border bg-card p-4"><div className="text-xs uppercase tracking-widest text-muted-foreground">{label}</div><div className="mt-2 font-medium">{value}</div></div>)}</div><div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-4 text-sm">Restricted production MVP: use external NVIDIA inference only for PUBLIC and synthetic/test INTERNAL content. Do not use FCI, CUI, customer-sensitive, or regulated production data. AI may assist but never approves a human gate.</div></div>;
}
