...
| 2     | Repository intelligence | GitRepository + AppRepositoryLink + GitCommitEvent + GitWorkflowRun, GitHub PAT client, `/api/webhooks/github` (HMAC verified), `/admin/repositories`, `/admin/repositories/commits`, `/admin/repositories/workflow-runs`, three new risk rules (production_behind_main, failed_workflow, security_sensitive_change). Sync runs as part of `runReconciliation()` when `ENABLE_GITHUB_SYNC=true`. |
| 3     | Deployment intelligence | RailwayResource + DeploymentSnapshot, Railway GraphQL client, `/api/webhooks/railway` (HMAC or query-secret verified), `/admin/ops/deployments`, `/admin/ops/health`, four new risk rules (failed_deployment, crashed_deployment, stale_deployment, missing_railway_mapping). Sync runs as part of `runReconciliation()` when `ENABLE_RAILWAY_SYNC=true`. |
| 4     | **Polish + intelligence** *(this PR)* | CommitSummary + AppDependency, ecosystem graph (`/admin/ops/ecosystem`), release notes (`/admin/repositories/release-notes`) with deterministic generator + optional AI augmentation gated on `ENABLE_AI_SUMMARIES`, full triage view (`/admin/ops/risk`), subdomain registry (`/admin/subdomains`), integration status pages (`/admin/integrations/{railway,github,cloudflare}`). Slice 4 closes out the Command Center surface. |
